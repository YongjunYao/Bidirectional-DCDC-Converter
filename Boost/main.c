...
int main()
{
  ...
  TIM1_PWM_Init();	
	...
	filter();
	Ek_1 = Ek;
	Ek = (int)(ADC_Filtered_Value[2] - 1062);	
  EkL_1 = EkL;
	EkL = (int)(ADC_Filtered_Value[1] - 1600);
  ...
  Ek = (int)(ADC_Filtered_Value[2] - 1062);
	...
	while(1)
	{
		...
		filter();
/*		
		if( (ADC_Filtered_Value[0] < 800) || (ADC_Filtered_Value[0] > 2000) )//BoostL Low input protection
		{
			TIM4->CCR4 = 1;
			TIM1->CCR2 = 1;
...
*/    
		...
		EkL_1 = EkL;
		EkL = (int)(ADC_Filtered_Value[1] - 1600);
    if (ADC_Filtered_Value[1] > 1600)//60V
		{
			GPIO_ResetBits(GPIOC, GPIO_Pin_13);
			if(ADC_Filtered_Value[1] > 2000)
			{
				TIM4->CCR4 = 75;
			}
			else
			{
				if((TIM4->CCR4 - EkL/60 - (EkL - EkL_1)/20) < 75)
				{
					TIM4->CCR4 = 75;
				}
				else
				{
					TIM4->CCR4 = TIM4->CCR4 - EkL/60 - (EkL - EkL_1)/20;
				}	
			}
		}		
		else
    {			
			GPIO_SetBits(GPIOC, GPIO_Pin_13);
			if((TIM4->CCR4 -EkL/60 - (EkL - EkL_1)/20) > 1200)
			{
				TIM4->CCR4 = 1200;
			}
			else
		...
		if(ADC_Filtered_Value[1] > 1200 && ADC_Filtered_Value[1] < 1800)
		{
			Next_Boost ++;
		}
		else
...
/*	
		if(ADC_Filtered_Value[1] < 700)//BoostH Low input Protection
		{
			Next_Boost = 0;
			TIM1->CCR2 = 75;
			continue;
...
*/
    Next_Boost = 1024;		
		if(Next_Boost >= 1000)//Ready for 1000 periods 
		{
			Next_Boost = 1024;//Prevent over range
			    //BoostH
			
			Ek_1 = Ek;
			Ek = (int)(ADC_Filtered_Value[2] - 1062);
			
			if (ADC_Filtered_Value[2] > 1062)
			{
				GPIO_ResetBits(GPIOC, GPIO_Pin_13);
				if(ADC_Filtered_Value[2] > 1500)
				{
					TIM1->CCR2 = 60;
				}
				else
				{
					if(( TIM1->CCR2 - Ek/120 - (Ek - Ek_1)/40) < 60)
					{
						TIM1->CCR2 = 60;
					}
					else
					{
						TIM1->CCR2 = TIM1->CCR2 - Ek/120 - (Ek - Ek_1)/40;
					}	
				}
			}		
			else
			{			
				GPIO_SetBits(GPIOC, GPIO_Pin_13);
				if((TIM1->CCR2 - Ek/120 - (Ek - Ek_1)/40) > 1200)
				{
					TIM1->CCR2 = 1200;
				}
				else
